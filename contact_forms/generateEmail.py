        ...
        with open(form_mail_path, "r") as f:
            ...
            for index, line in enumerate(lines):
                for field in all_fields:
                    if field in line:
                        random_field = [random_item for random_item in random_fields if random_item['name'] == field]
                        if len(random_field) > 0:
                            lines[index] = lines[index].replace(f"[{field}]", random.choice(random_field[0]['value']))
