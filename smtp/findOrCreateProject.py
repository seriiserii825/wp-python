...
def findOrCreateProject():
    ...
    WebDriverWait(driver, 3000).until(
        EC.presence_of_element_located((By.CSS_SELECTOR, table_rows))
    )
    rows = driver.find_elements(By.CSS_SELECTOR, table_rows)
    for row in rows:
        project_title = smtp.settings.project_title
        project_name = row.find_element(
            By.CSS_SELECTOR, "a.cfc-purview-picker-list-name-link"
        )
        ...
        if project_title == project_name_text:
            ...
            create_project_button = driver.find_element(
                By.CSS_SELECTOR, "button.purview-picker-create-project-button"
            )
            create_project_button.click()
            search_input = driver.find_element(
                By.CSS_SELECTOR, "proj-name-id-input input.mat-mdc-input-element"
            )
            search_input.clear()
            search_input.send_keys(project_title)
            create_project_button = driver.find_element(
                By.CSS_SELECTOR, "button.projtest-create-form-submit"
            )
            time.sleep(2)
            create_project_button.click()
