...
def getLayoutType(type): return next(
...
class CreateFile:
    def __init__(self, type: str, selected_dir=None):
        ...
        if not os.path.exists(str(self.dir_name)):
        ...
        self.create_file_input_placeholder = getLayoutType(
            type)['create_file_input_placeholder']
    ...
    def getFileName(self, file_name=''):
        if file_name != '':
            ...
            self.file_name = input(
                f"Enter file name like {self.create_file_input_placeholder}: ")
    ...
    def changeClassName(self, file_path):
        os.system(
            f"sed -i -e 's/{self.layout_text}/{self.file_name}/g' '{file_path}' ")
    ...
    def includeInPage(self):
        if self.type == 'php':
            ...
            self.insertBeforeLastLine(
                selected_file, f"<?php get_template_part('{self.dir_name}/{self.selected_dir}/{self.file_name}');?>\n")
